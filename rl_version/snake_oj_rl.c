...
static char infer_base_cell(int r, int c)
{
	...
	if (map[r][c] != 'H' && map[r][c] != 'B' && map[r][c] != 'F')
		return original_map[r][c];
...
static int is_food_reachable(void)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (nr < 0 || nr >= ROWS || nc < 0 || nc >= COLS) continue;
		if (visited[nr][nc]) continue;
		if (nr == food_r && nc == food_c) return 1;
		visited[nr][nc] = 1;
		q_r[qt] = nr;
		q_c[qt] = nc;
		qt++;
...
static int select_q_move(void)
{
	...
	/*
	 * 只在明确需要全局安全策略接管的局面禁用 Q：
	 * 1. 低 N 迷宫以及 N=32 的门型列迷宫里，现有硬编码路径明显比 Q 表更稳定；
	 * 1. 高密度局面需要主动延迟吃食，优先保留追尾空间；
	 * 2. 高 N 且障碍较多时，局部 Q 偏好容易在细长通道中空转。
	 */
	if (N <= 16 || N == 32)
		return -1;
	if (should_delay_food_for_density())
		return -1;
	if (N >= 128 && obstacle_count >= 30)
		return -1;
